...
| Adoption feedback | The abstraction needs [two adoption contexts](docs/project/12-roadmap.md) before 1.0 — real friction reports are worth more than features |
| Answers to open questions | Comment on the relevant OQ in [the decision log](docs/project/13-open-questions.md); a PR resolving one must update that file in the same change |
| Docs fixes | Direct PR |
| Implementation | Claim a milestone-scoped issue ([implementation plan](docs/project/14-implementation-plan.md)); read [the maintainer directive](docs/project/17-maintainer-directive.md) first — it is the standing execution contract |
...
2. **Decisions are recorded, not remembered.** Behavior invented without an entry in the [decision log](docs/project/13-open-questions.md) is not mergeable. Correcting a settled decision means a new decision record (see [the Spec Corrections RFC](docs/project/18-spec-corrections-rfc.md) for the shape).
...
5. **No LLM in the test suite.** The surface is a typed contract; contracts are tested deterministically ([Testing](docs/08-testing.md)). Model-based evaluation may supplement conformance tests, never replace them.
