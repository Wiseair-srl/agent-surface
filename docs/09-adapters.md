...
> **D41 authority boundary:** supported provider/MCP assembly passes compiled tools through `createAgentExposureGateway(authority)`. Raw appended tools are rejected. Registry-backed adapters inherit the same guarantee because registration already verified private proof and runtime semantics.
...
export function createWebMcpAdapter(options?: {
  snapshotContext?: Omit<SnapshotContext, "consumer">;
  /** Curate presentation only; return null to skip a capability. */
  exposeCapability?: (
...
- execution is always generated by the adapter and routed through `registry.invoke`; the curation hook cannot supply or replace `execute`;
