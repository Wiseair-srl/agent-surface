...
> [!NOTE]
> The frontend is never a security boundary. agent-surface makes browser exposure explicit, minimal, and auditable; the server remains authoritative.

The model in one paragraph: nothing is exposed unless registered; what is exposed is filtered per consumer by **policies** at discovery and re-checked at invocation; anything dangerous demands **single-use confirmation evidence** bound to the exact inputs the user saw; every step emits **audit events**; and for domain operations all of that is UX layered *on top of* server enforcement, never instead of it. If you review one thing before shipping, make it the [threat model table](#threat-model) and the [hide-vs-disable rule](#hide-vs-disable).
...
6. **Authority = registration + policies + authenticated context** — invocation pipeline phases 2–6 ([Architecture](02-architecture.md#invocation-pipeline)).
7. **Server always re-validates** — the executor forwards through the user's session; see [oRPC client and server responsibilities](05-orpc-integration.md#client-and-server-responsibilities).
...
11. **Observation minimization + authorization** — policies apply to observations; output caps; hide-vs-disable rule; authoring norms ([React API anti-patterns](04-react-api.md#anti-patterns)).
...
export interface AgentPolicy {
   ...
   * NO agent input is available here, by type construction.
...
- **Two stages, one boundary.** Phase 4 (`onAuthorize`) runs before input processing and cannot observe agent input. Phase 6 (`onInvoke`) runs after the effective input is constructed and validated, and observes only that value. Confirmation is decided at phase 6 over the effective input.
...
- Policies MUST NOT mutate input. There is no input-rewriting middleware; validated effective input remains stable through the policy chain and audit trail.
...
All built-ins run in core without external dependencies. Host semantics, including what "authenticated" means, are injected rather than assumed.
...
1. **Representation.** `CONFIRMATION_REQUIRED` is a typed error result (`retry: "with-confirmation"`) carrying `details: { confirmationId, summary, expiresAt, effect }`. It is a protocol step, not a failure. It is produced after the effective input is validated; a malformed binding or supplied locked field fails before any record is created.
2. **Correlation.** The pending record binds to a canonical request digest over `{ surfaceId, registrationId, capabilityId, consumerKey, effectiveInput, effect }`. Canonical JSON sorts object keys, preserves array order, omits `undefined`, accepts finite numbers, and normalizes `-0` to `0`. A retry MUST produce the identical digest and exact effective input or return `CONFIRMATION_INVALID { reason: "mismatch" }`. Evidence cannot be replayed across a consumer, registration, capability, or registry instance.
...
9. **UX modes.** The two-phase flow above is transport-friendly and auditable. The embedded toolset's `confirmations: "wait"` mode performs the wait and retry internally so the model sees one call and one result. Semantics and audit are identical. Mode selection follows the declared [adapter topology](09-adapters.md#confirmation-topology).
10. **Bounded store.** At most `limits.maxPendingConfirmations` records may be pending. An invocation that would exceed the limit returns `RATE_LIMITED {reason: "queue-full"}` without creating a record.
...
- Same-realm JavaScript can lie about `origin`. Real containment requires an out-of-realm boundary such as an iframe or worker. The guard is a policy tool, not a sandbox.
...
`environment: "development"` adds `consoleAuditSink()` alongside the in-memory one. It writes on the **diagnostic** stream, never stdout (`AS-OBSV-002`): `console.debug` in the browser and `console.error` under Node. An audit trail must not corrupt [CLI output](20-cli.md#output).
