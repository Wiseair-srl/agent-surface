...
Everything is written with RFC-style **MUST/SHOULD/MAY** where behavior is normative, and labeled **Draft / Experimental / Future**. The specification is implemented and published as 0.x; where the docs describe something that is *not* built — a deferred decision, a topology without a runnable example — they say so in place rather than implying coverage.
