...
### v0.10 — surface coverage, then adoption and enforcement

**Surface coverage** ([21](21-surface-coverage-rfc.md), D35–D37, manifest 103 → 111/111). `inspect` answered *what can an agent do here right now* and was quietly read as answering *did we author something no scenario reaches* — a question nothing in the repository could compute. The gap came from inheriting "the catalog is undiscoverable" from "the projection is dynamic": true of the projection, false of the catalog, since `type` is a string literal and capability names are object keys.

- **A static inventory** (`AS-COVER-001…003`, `AS-COVER-006`, D35). `agent-surface capabilities` reads the TypeScript program — no Vite server, no jsdom, no scenarios, no mount — and emits one entry per authored capability with `resolution: static | partial | unresolved`. An unreadable call site is reported with its file, line and the construct that defeated the extractor, and the command **exits non-zero** unless `--allow-unresolved` is passed: a partial understanding of a codebase that reports itself as complete is the failure the whole slot exists to remove. It reads code and creates nothing — directive §2.1 is untouched, and `AS-COVER-006` keeps the inventory off the package root adapters import, mirroring `AS-EXPLAIN-004` ([20 §capabilities](20-cli.md#capabilities)).
- **`coverage`: authored minus reached** (`AS-COVER-004…005`, D36). Joins the inventory against the union of every scenario's *explanation* — not its snapshot, because a policy-hidden capability **was** reached and counting it as a gap would have made the example app's `anonymous` scenario contribute eleven false ones. Adoption ratchets through a committed `.agent-surface/coverage-allow.json` whose stale entries fail the command ([20 §coverage](20-cli.md#coverage)).
- **The CLI stopped asserting completeness it could not back** (`AS-CLI-006…007`, D37). A rejected registration — duplicate `(type, instanceId)`, or an `onRegister` guard — reached neither the snapshot, nor the explanation, nor any baseline, and its only diagnostic went through `devError`, which prints nothing under the `environment: "test"` the documented config shape builds. The registry had always emitted `component-rejected`; the collector now reads it. Counts name their scenario and scope, `hidden` prints unconditionally (signed out, the example app rendered `0 callable, 0 visible-disabled` over eleven capabilities authority had hidden, under the words *nothing is registered*), and a green `check` names the scenarios it compared.
...
**Adoption and enforcement**, carried from v0.9:
